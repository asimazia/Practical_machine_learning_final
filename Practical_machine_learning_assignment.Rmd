...
From the result, we can see that both the random forest and general boosting model perform better than the decision tree. The random forest model slightly shows better accuracy than the generalized boosted model. Also, the out of sample error is ~ 0.18% that is good enough to use this model for the quiz section. So for the quiz section, I will use the random forest model on the Test dataset given for the assignment quiz.
```{r, echo= TRUE}
predictTEST <- predict(RF, newdata=testing)
predictTEST
```